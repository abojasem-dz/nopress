...
class Akismet {
	...
	public static function transition_comment_status( $new_status, $old_status, $comment ) {
		...
		if (
			 ...
			 // spam=1: Clicking "Spam" underneath a comment in panel and allowing the AJAX request to happen.
			 || ( isset( $_POST['spam'] ) && (int) $_POST['spam'] == 1 )
			 // unspam=1: Clicking "Not Spam" underneath a comment in panel and allowing the AJAX request to happen. Or, clicking "Undo" after marking something as spam.
			 ...
			 // action=spam: Choosing "Mark as Spam" from the Bulk Actions dropdown in panel (or the "Spam it" link in notification emails).
			 // action=unspam: Choosing "Not Spam" from the Bulk Actions dropdown in panel.
			 // action=spamcomment: Following the "Spam" link below a comment in panel (not allowing AJAX request to happen).
			 // action=unspamcomment: Following the "Not Spam" link below a comment in panel (not allowing AJAX request to happen).
			 || ( isset( $_GET['action'] ) && in_array( $_GET['action'], array( 'spam', 'unspam', 'spamcomment', 'unspamcomment', ) ) )
			 // action=editedcomment: Editing a comment via panel (and possibly changing its status).
